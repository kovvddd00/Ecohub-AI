  ...
  return (
    <div className="space-y-5 max-w-[1200px]">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold font-display tracking-tight text-white">Reports</h1>
          <p className="text-xs text-[#6B7280] mt-1">Review results from models trained during this session.</p>
        </div>
      ...
      {resultsHistory.length === 0 ? (
        <div className="card-base flex flex-col items-center justify-center py-16 text-center">
          ...
          <h3 className="text-sm font-semibold text-white mb-1">No Models Trained</h3>
        ...
        <div className="space-y-3">
          {resultsHistory.map((item) => (
            <div key={item.id} className="card-base p-4 flex flex-col gap-4">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-[#6B7280]">
                    ...
                    <span className="font-medium text-[#9CA3AF] max-w-[200px] truncate">{item.dataset}</span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-4 gap-3 border-t border-[#1F2937] pt-3">
                {Object.entries(item.metrics).map(([name, val], i) => {
                  ...
                  return (
                    <div key={i} className="bg-[#0d1526] p-2.5 rounded-lg border border-[#1F2937]">
                      <p className="text-[9px] text-[#6B7280] uppercase font-semibold tracking-wider">{name}</p>
